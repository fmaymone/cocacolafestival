...
public class Video4 extends VideoGerado {

	private String nomeGerar;
	private String idVideoTemp;

	public Video4(CocaCola coca) throws Exception {
		this.setIdVideo(4);
		this.setgPosicoes(new GeradorPosicoes(4));
	...
	String idVideoTemp = "4";
	
	
	//IMediaReader mediaReader = ToolFactory.makeReader(prop
		//	.getUrlVideosRespostas()[0]);
	ClassLoader classLoader = getClass().getClassLoader();
	//File file = new File(classLoader.getResource(prop.getProp().getProperty("prop.pergunta.4.resposta")).getPath());
	File file = new File(classLoader.getResource("videos/video4.mp4").getPath());
	...
	String urlTemp = folder.getAbsolutePath() +  "/"+ super.getCocaCola().getNome() + "/"+"video4.mp4"; 

	IMediaWriter mediaWriter = ToolFactory.makeWriter(urlTemp, mediaReader);

	String nomeGerado = "Video4	";
	...
	System.out.println("------------------------Gerando Video 4 -------------- ");
	while (mediaReader.readPacket() == null)
		;

	System.out.println("------------------------Video 4 GERADO-------------- ");
